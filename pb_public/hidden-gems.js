...
class HiddenGemsManager {
    ...
    setupMapInteraction() {
        ...
        this.map.getMap().on('contextmenu', (e) => {
            e.originalEvent.preventDefault();
            this.openGemForm(e.latlng);
        });

        // Long press for touch devices using native pointer events
        let pressTimer = null;
        let longPressTriggered = false;

        this.map.getMap().on('pointerdown', (e) => {
        ...
        this.map.getMap().on('pointerup', () => {
            clearTimeout(pressTimer);
        });

        this.map.getMap().on('pointermove', () => {
            if (!longPressTriggered) {
                clearTimeout(pressTimer);
            }
        });

        this.map.getMap().on('pointercancel', () => {
            clearTimeout(pressTimer);
        });
    }
    ...
    createGemForm() {
        ...
        const container = modal.querySelector('.gem-form-container');
